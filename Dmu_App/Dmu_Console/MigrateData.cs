...
namespace Dmu_Console
{
    public class MigrateData
    {
        ...
        public static async Task BatchMigration(int start, int End, CancellationTokenSource s_cts, MigrationStatus status)
        {
            ...
            Console.WriteLine("Migration Application started.");
            ...
            tempstart = start;
            while (totalDataRows >= UniversalConstant.limitofTransfer)                               //675  6 batch 100 75 1 batch
            {
            ...
            {
                status.Status = EnumValues.Faulted;
                Console.WriteLine("-------------------------------------------------------------------------------");
                Console.WriteLine($"Some Data in this  Range of Data From {start} - {End} is Already Been Migrated");
                Console.WriteLine("-------------------------------------------------------------------------------");
            ...
            Console.WriteLine("Migration Application ended .....");
        }
        ...
        public static async Task ProcessUrlAsync(int start,int end, CancellationToken token)
        {
            ...
            {
                //Console.WriteLine($"Migrating From {start} To {end} Rows to Destination Table");
                List<SourceModel> sourceDataBatch = new List<SourceModel>();                                //100
                if(!token.IsCancellationRequested)
                {
